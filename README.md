...
**详细档下，组体展开时组头只留标签。** 详细档的组头把实时细节接在标签后面（`正在分析请求 · …`），而那一段正是组内思考行正在出的字——两处一起出字看着像重影。所以组体开着的时候，组头只显示标签那半截：组头、图标、开合控件都还在原处，只是不再跟着出字；组体一收起，原文本立刻回来。简洁档的组头没有这一段，标记不成立，什么都不变。

标签是 process-fold 按分隔符从组头文本里切出来、写到 `data-chat-ux-label` 上的，样式表用 `::after` 显示它——detail 与标签在同一个文本节点里，CSS 切不开，只能让原文本整块让位。代价是这半截标签不带 dsh 的流光动画（那层动画挂在原元素上）；组体既然已经展开、内容就在下面，这一点不明显。
