...
- **聊天气泡动效**（beta，默认关）：打开之后，提交的那条气泡从输入框飞上去、落进消息列，整段默认 200 ms，可在插件页上改。
...
| 组体跟随 | 随「增强跟随」 | 不是单独一项：卡片上只有「增强跟随」那个开关，它同时控制限高过程组里思考与工具输出是否掉队。 |
| 插入符动效 | 无论何时 | 三档：关／移动时／无论何时。 |
| 聊天气泡动效 | 关（beta） | 打开后，提交的那条气泡从输入框飞上去、落进消息列。 |
| 发送动效时长 | 200 | 气泡起飞那一段的整段时长，80–1200 的整数毫秒；上面那个开关关着时停用。 |
| 自带字体 | 开 | 关掉后走系统字体栈。 |
