...
- 「工作步骤展示」（设置 → 通用）的 `compact` 与 `standard`（默认）两档下，**运行中的过程组体本身是收起的**，自动展开的思考行要等读者展开那一组才看得到；`detailed` 与 `verbose` 两档的组体直接可见，效果立刻可见。四档都不自动展开单个思考行，这正是本插件仍然有用的前提。
- 展开是模拟点击（`click()`）实现的，dsh 若改掉 `DisclosureRow` 的交互，这里会静默失效——不报错，只是不再自动展开。

## 过程组自动开合

dsh 把一轮里相邻的过程内容（思考、工具、命令、写文件……）收进一个**过程组**。「工作步骤展示」的**简洁**与**标准**（默认）两档下，组体一开始是收起的——模型正在做什么，读者得先点开那一行才看得见。本插件让它在过程还在跑时开着，这一段过程结束（最终正文该出来了）时收回去。
...
**带实时细节的档位下，组体展开时组头只留标签。** 标准与详细两档的组头把这一段的细节接在标签后面（`正在分析请求 · …`），而那一段正是组内思考行正在出的字——两处一起出字看着像重影。所以组体开着的时候，组头只显示标签那半截：组头、图标、开合控件都还在原处，只是不再跟着出字；组体一收起，原文本立刻回来。简洁与完全展开两档的组头没有这一段，标记不成立，什么都不变。

标签是 process-fold 按分隔符从组头文本里切出来、写到 `data-chat-ux-label` 上的，样式表用 `::after` 显示它——detail 与标签在同一个文本节点里，CSS 切不开，只能让原文本整块让位。

那层流光原本挂在被隐藏的元素上，所以替上来的这半截自己带一份：同一道渐变、同一个周期（1.5 s），半宽也按 dsh 的算法算——`process-fold` 把「标签字符数 × 8px」写进 `--dsh-chat-ux-label-spread`，与 `TextShimmer` 内联的是同一个量。只有 keyframes 的名字是自己的：宿主那份样式走 CSS Modules，动画名随时会被构建改名，不能当接口用。规则包在 `@supports` 里——撑得起这道渐变的浏览器才把文字交给背景画，撑不起就退回一段静态标签，不会配着一个画不出来的背景把字也搭进去。

替上来的文本是伪元素内容，不进可访问性树，所以 `process-fold` 同时把标签写进组头的 `aria-label`——不然读屏拿到的是一个没有名字的按钮。这一条只在组头真带着实时细节时挂着，过程结束后就摘掉。

**「详细」与「完全展开」两档下本插件没有可做的事**：详细档只给已经结束的轮次画组头、运行中的组体本来就开着，完全展开档连组头都不画——两档里都没有可切换的组。
